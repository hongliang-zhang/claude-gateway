...
class Config:
    def __init__(self):
        self.gateway_auth_mode = os.environ.get("GATEWAY_AUTH_MODE", "shared").strip().lower()
        if self.gateway_auth_mode not in ("shared", "pass_through"):
            raise ValueError("GATEWAY_AUTH_MODE must be 'shared' or 'pass_through'")

        self.openai_api_key = os.environ.get("OPENAI_API_KEY")
        if self.gateway_auth_mode == "shared" and not self.openai_api_key:
        ...
        if not self.anthropic_api_key and self.gateway_auth_mode == "shared":
        ...
        self.posthog_logs_enabled = os.environ.get("POSTHOG_LOGS_ENABLED", "false").lower() in (
            "1",
            "true",
            "yes",
            "on",
        ...
        self.posthog_capture_bodies = os.environ.get("POSTHOG_CAPTURE_BODIES", "false").lower() in (
            "1",
            "true",
            "yes",
            "on",
...
try:
    config = Config()
    api_key_state = "configured" if config.openai_api_key else "pass-through"
    print(f" Configuration loaded: API_KEY={api_key_state}, BASE_URL='{config.openai_base_url}'")
