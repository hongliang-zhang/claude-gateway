...
class OpenAIClient:
    ...
    @staticmethod
    def raise_provider_error_if_present(response_data: Dict[str, Any]) -> None:
        """Detect OpenAI-compatible providers that wrap errors in HTTP 200 responses."""
        if response_data.get("success") is not False:
            return

        status_code = response_data.get("code") or 502
        try:
            status_code = int(status_code)
        except (TypeError, ValueError):
            status_code = 502

        message = response_data.get("msg") or response_data.get("message") or "Provider error"
